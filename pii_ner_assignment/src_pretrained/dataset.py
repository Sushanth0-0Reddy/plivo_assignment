...
from labels import assignment_label_to_hf
            ...
            for line in f:
                ...
                for (start, end) in offsets:
                    if start == end or start >= len(char_tags):
                        hf_tags.append("O")
                        continue
                    assignment_tag = _strip_bio(char_tags[start])
                    hf_label = assignment_label_to_hf(assignment_tag)
                    hf_tags.append(hf_label if hf_label in self.label2id else "O")

                if len(hf_tags) != len(input_ids):
                    hf_tags = ["O"] * len(input_ids)

                label_ids = [self.label2id.get(t, self.label2id["O"]) for t in hf_tags]
